...
export const repoIndexFiles = pgTable(
  ...
  (table) => ({
    repoIndexFilesProjectContentHashIdx: index("repo_index_files_project_id_content_hash_idx").on(
      table.projectId,
      table.contentHash,
    ),
  }),
...
export const repoIndexChunks = pgTable(
  ...
  (table) => ({
    repoIndexChunksEmbeddingHnswIdx: index("repo_index_chunks_embedding_hnsw_idx")
      .using("hnsw", table.embedding.op("vector_cosine_ops"))
      .with({ m: 16, ef_construction: 64 }),
    repoIndexChunksFileChunkIndexIdx: index("repo_index_chunks_file_id_chunk_index_idx").on(
      table.fileId,
      table.chunkIndex,
    ),
    repoIndexChunksContentTrgmIdx: index("repo_index_chunks_content_trgm_idx")
      .using("gin", table.content.op("gin_trgm_ops")),
    repoIndexChunksSymbolNameTrgmIdx: index("repo_index_chunks_symbol_name_trgm_idx")
...
export const repoIndexSymbols = pgTable("repo_index_symbols", {
  ...
  chunkId: uuid("chunk_id").references(() => repoIndexChunks.id, { onDelete: "set null" }),
});
